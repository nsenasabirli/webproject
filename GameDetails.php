...
<div class="dropdown position-fixed bottom-0 end-0 mb-3 me-3 bd-mode-toggle">
    ...
    <div class="container mt-5">
        ...
        if (isset($_GET['game_id'])) {
            ...
            $sql = "SELECT * FROM wowgames WHERE game_id = ?";
            ...
            if ($result->num_rows > 0) {
                ...
                <div style="background-color: beige;" class="card mb-3">
                    ...
                    <div class="col-md-4">
                    <a href="'.$row["bgg_url"].'">
                    <img src="'.$row["image_url"].'" class="img-fluid rounded-start" alt="'.$row["title"].'" style="display: block; margin: auto;" ></a>
                    <a href="R-Favourites.php?username='.$user.'&game_id='.$row["game_id"].'" class="btn btn-primary mr-2" style="display: block; margin: auto; width: 40%">Add to Favourites</a>
                     </div>
                        <div class="col-md-8">
                          <div style="border: 1px solid black; padding: 20px;">
                            <div class="card-body">
                                <h3 style="font-family: cursive" class="card-title">'.$row["names"].'</h3>
                                <p class="card-text"><strong>Minimum Players: </strong> '.$row["min_players"].'</p>
                                <p class="card-text"><strong>Maximum Players:</strong> '.$row["max_players"].'</p>
                                <p class="card-text"><strong>Rating:</strong> '.$row["avg_rating"].'</p>
                                <p class="card-text"><strong>Age:</strong> '.$row["age"].'</p>
                                <p class="card-text"><strong>Playing Style:</strong> '.$row["mechanic"].'</p>
                                <p class="card-text"><strong>Category:</strong> '.$row["category"].'</p>
                                <p class="card-text"><strong>Game Designer:</strong> '.$row["designer"].'</p>
                                <p class="card-text"><strong>Net Weight:</strong> '.$row["weight"].'</p>
                            </div>
                        </div>
                    </div>
                </div>';
